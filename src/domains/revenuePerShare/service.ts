...
export const calculateRevenuePerShare = async (query: RevenuePerShareQuery): Promise<RevenuePerShareResult> => {
  ...
  // 存進 oingg-analysis DB 的 revenue_per_share_result，供之後查歷史紀錄用。存檔失敗不應該讓已經算好的結果回傳失敗。
  try {
    ...
    console.error('[revenue-per-share]: 寫入 revenue_per_share_result 失敗，不影響本次回傳結果。', error);
  }
